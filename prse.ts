...
class Parser<T> {
	...
	parse = (v: any): T => {
		try {
			...
			if (e instanceof ValidationError && !e.message.startsWith(this.errorMessage)) {
	...
	notEmpty = (): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'string' || Array.isArray(v)) {
				...
				throw new ValidationError('Invalid type for notEmpty validation');
			}
			return v;
		});
	};
	...
	or = (...parsers: Parser<T>[]): Parser<T> => {
		return new Parser<T>((v: any) => {
			...
			throw new ValidationError('No valid alternatives');
		});
	};

	and<U>(other: Parser<U>): Parser<[T, U]> {
		return new Parser<[T, U]>((v: any) => {
			const result1 = this.parse(v);
			const result2 = other.parse(v);
	...
	not<U>(other: Parser<U>): Parser<[T, U]> {
		return new Parser<[T, U]>((v: any): any => {
			...
			let result2;
			try {
				result2 = other.parse(v);
				result2 = false;
			} catch (e) {
				result2 = true;
			}
			if (result2 == false) {
				throw new ValidationError('Failed to pass not(condition)');
			}
			return [result1, result2];
		});
	}
	...
	range = (min: number, max: number): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'number') {
				...
				throw new ValidationError('Invalid type for range validation');
			}
			return v;
		});
	};
	...
	lessThan = (max: number): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'number') {
				...
				throw new ValidationError('Invalid type for lessThan validation');
			}
			return v;
		});
	};
	...
	moreThan = (min: number): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'number') {
				...
				throw new ValidationError('Invalid type for moreThan validation');
			}
			return v;
		});
	};
	...
	lessThanOrEqualTo = (max: number): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'number') {
				...
				throw new ValidationError('Invalid type for lessThanOrEqualTo validation');
			}
			return v;
		});
	};
	...
	moreThanOrEqualTo = (min: number): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'number') {
				...
				throw new ValidationError('Invalid type for moreThanOrEqualTo validation');
			}
			return v;
		});
	};
	...
	conditional = <R>(predicateFn: (v: T) => boolean, trueParser: Parser<R>, falseParser: Parser<R> = unknown()): Parser<R> => {
		return new Parser<R>((v: any) => {
			const result = this.parse(v);
			return predicateFn(result) ? trueParser.parse(result) : falseParser.parse(result);
		});
	};
	...
	union = (...parsers: Parser<any>[]): Parser<T> => {
		return new Parser<T>((v: any) => {
			...
			throw new ValidationError('Value does not match any of the union types');
		});
	};
	...
	combine = <R>(parser: Parser<R>): Parser<R> => {
		return new Parser<R>((v: any) => {
			const result1 = this.parse(v);
			const result2 = parser.parse(result1);
			return result2;
		});
	};
			...
			if (typeof v === 'string') {
				...
				for (const item of v) {
					if (typeof item === 'string') {
						...
						throw new ValidationError('Array contains non-string elements');
					}
				}
			} else if (typeof v === 'object' && v !== null) {
				for (const value of Object.values(v)) {
					if (typeof value === 'string') {
						...
						throw new ValidationError('Object contains non-string values');
					}
				}
			} else {
				throw new ValidationError('Invalid type for pattern validation');
			}
			return v;
		});
	};
	...
	minLength = (min: number): Parser<T> => {
		return new Parser<T>((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('Invalid type for minLength validation');
			}
			return this.parse(v);
		});
	};
	...
	maxLength = (max: number): Parser<T> => {
		return new Parser<T>((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('Invalid type for maxLength validation');
			}
			return this.parse(v);
		});
	};
	...
	length = (length: number): Parser<T> => {
		return new Parser<T>((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('Invalid type for length validation');
			}
			return this.parse(v);
		});
	};
	...
	oneOf = (allowed: any[]): Parser<T> => {
		return new Parser<T>((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('Invalid type for oneOf validation');
			}
			return this.parse(v);
		});
	};
	...
	first = (substring: string): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('startsWith method only works with strings, arrays, or objects');
			}
			return v;
		});
	};
	...
	last = (substring: string): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('endsWith method only works with strings, arrays, or objects');
			}
			return v;
		});
	};
	...
	includes = (substring: string): Parser<T> => {
		return this.map((v: any) => {
			if (typeof v === 'string') {
				...
				throw new ValidationError('includes method only works with strings, arrays, or objects');
			}
			return v;
		});
	};
	...
	every = (parser: Parser<any>): Parser<T> => {
		return new Parser<T>((v: any) => {
			if (Array.isArray(v)) {
				...
				throw new ValidationError('Every method only works with arrays or objects');
			}
			return this.parse(v);
		});
	};
	...
	some = (parser: Parser<any>): Parser<T> => {
		return new Parser<T>((v: any) => {
			...
			if (Array.isArray(v)) {
				...
				throw new ValidationError('Some method only works with arrays or objects');
			}
			if (!isValid) {
				throw new ValidationError('None of the elements/values match the parser');
			}
			return this.parse(v);
		});
	};
		...
		if (Object.keys(v).length == 0) {
			...
			for (const key in schema) {
				if (v.hasOwnProperty(key)) {
					result[key] = schema[key].parse(v[key]);
				} else {
...
const array = <T>(elementParser: Parser<T>): Parser<T[]> =>
	new Parser<T[]>((v: any) => {
		...
		return v.map(elementParser.parse);
	});
...
const record = <T>(vParser: Parser<T>): Parser<{ [key: string]: T }> =>
	new Parser<{ [key: string]: T }>((v: any) => {
		...
		for (const key in v) {
			result[key] = vParser.parse(v[key]);
		}
		return result;
	});
...
const set = <T>(elementParser: Parser<T>): Parser<Set<T>> =>
	new Parser<Set<T>>((v: any) => {
		...
		return new Set(Array.from(v).map(elementParser.parse));
	});
...
const map = <K, V>(keyParser: Parser<K>, vParser: Parser<V>): Parser<Map<K, V>> =>
	new Parser<Map<K, V>>((v: any) => {
		...
		v.forEach((v: any, k: any) => {
			result.set(keyParser.parse(k), vParser.parse(v));
		});
		return result;
	});
...
const tuple = <T extends any[]>(parsers: {
	...
	new Parser<T>((v: any) => {
		...
		return parsers.map((parser, i) => parser.parse(v[i])) as T;
	});
...
function prse(parser: any, rawData: string, onSuccess: (() => void) | null = null, onFailure: ((errorInfo: string, errorDetails: { name: string; message: string; file: string; line: string; column: string }) => void) | null = null): boolean {
	try {
		...
		const t = `\x1b[31m${e.name}\x1b[0m: \x1b[34m${e.message}\x1b[0m\n\x1b[33mFile\x1b[0m: \x1b[36m${i.file}\x1b[0m\n\x1b[33mAt [prse(...)]\x1b[0m: \x1b[36mLine - \x1b[0m\x1b[35m${i.line}\x1b[0m; \x1b[36mColumn - \x1b[0m\x1b[35m${i.column}\x1b[0m`;
		if (onFailure !== null) {
			onFailure(t, {
				name: e.name,
				message: e.message,
