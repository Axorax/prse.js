...
function modifyFileForExports(filePath) {
	...
	content = content.split('\n').map((line, index) => index === 0 ? `export ${line}` : line).join('\n');
	content += '\nexport { assert, Parser, string, number, boolean, unknown, object, objectLoose, array, record, set, map, tuple, enums, fail, date, instance, func, prse, symbol, regexp, bigInt, int8Array, uint8Array };';
