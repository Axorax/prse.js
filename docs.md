...
`prse()` requires two arguments and can take in four arguments. The 1st one is the format or parser. 2nd one is the data. 3rd one is a function that will run if the data is correct. 4th one is a function that will run if the data is incorrect. You can get the error and the error info in JSON format from it.

By default, PRSE will console.error() the test that failed. If you provide a function in the 4th argument then it will only run that and won't console.error() anything.

> [!NOTE]
> Instead of calling `prse(...)` you can also use something like this but it is not recommended as it is much worse:
>
> ```js
> try {
> 	format.parse(data);
...
> [!IMPORTANT]
> From now on, in the examples, I will not be running the parser (`prse()`) as it will make the code larger. If you want to try out the code in the examples, make sure to run the parser.

## [2/2] Important: Separate array and object
...
Make sure to use the `prse()` function and log out if it is valid or not when using any example code and also provide the data to validate.

| Method | Description | For | Chainable | Example |
| --- | --- | --- | --- | --- |
| parse | parse data using provided format. Throws a `ValidationError` if parsing fails. | any | Yes | `string().parse("Hi")` |
| withMessage | custom error message | any | Yes | `string().withMessage("Invalid!").parse("Hi")` |
